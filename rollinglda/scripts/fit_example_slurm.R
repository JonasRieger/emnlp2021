...
# choose interval, memory and initial time period
interval = "quarter"
#interval = "year"
...
wd = paste0("rolling_", interval, "_", start, "_", "memory", memory)
...
b = Sys.time()
message(difftime(b, a, unit = "hours"), " hours")
lda = getLDA(init)
...
for(i in seq_len(length(cuts)-1)){
  ...
  message("### ", i, "/", length(cuts)-1, ", eta: ",
          (difftime(b, a, unit = "hours")/(i-1)) * (length(cuts)-2-i)," hours ###")
