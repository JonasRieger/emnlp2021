...
dir.create("init")
setwd("init")
dir.create(paste0(as.character(start), "complete"))
setwd(paste0(as.character(start), "complete"))
...
batch = LDABatch(docs, vocab, K = K, alpha = alpha, eta = eta, num.iterations = num.iterations,
                 resources = list(walltime = 8*60*60, memory = 8192))
